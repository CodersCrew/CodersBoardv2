...
export class ApplicantInvitation extends AbstractAggregateRoot<ApplicantInvitationId> {
    ...
    cancel() {
        ...
        this.apply(ApplicantInvitationDomainEvent.InvitationCancelled.newFrom(this.id, this.currentDate, {}))
    }

    onInvitationCancelled(event: ApplicantInvitationDomainEvent.InvitationCancelled) {
        this.id = event.aggregateId;
