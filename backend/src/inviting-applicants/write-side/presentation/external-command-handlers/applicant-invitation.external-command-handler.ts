...
export namespace ApplicantInvitationExternalCommandHandler {
  ...
  class InviteApplicantCommand
    ...
    async execute({
      ...
      return this.internalCommandSender.send(new ApplicantInvitationInternalCommand.InviteApplicantCommand(personalEmail, firstName, lastName))
    }
  }

  @CommandHandler(ApplicantInvitationPublicCommand.CancelApplicantInvitationCommand)
  class CancelApplicantInvitationCommand
    ...
    async execute({applicantInvitationId}: ApplicantInvitationPublicCommand.CancelApplicantInvitationCommand) {
      return this.internalCommandSender.send(new ApplicantInvitationInternalCommand.CancelApplicantInvitationCommand(applicantInvitationId))
    }
  }
