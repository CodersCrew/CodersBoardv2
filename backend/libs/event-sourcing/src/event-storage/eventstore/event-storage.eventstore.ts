    ...
    return this.httpService
        .post(`/streams/${eventStreamId.raw}`, eventsToStore, {
          headers: {
            ...
            catchError(err => {
              const eventStreamModifiedConcurrent = err.response.status === 400 && err.response.statusText === 'Wrong expected EventNumber';
              return eventStreamModifiedConcurrent ? throwError(errorCausedBy(new Error('EventStream modified concurrently!'), err)) : throwError(err);
            })
        )
  }
